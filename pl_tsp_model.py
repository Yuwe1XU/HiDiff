...
from utils.tsp_red_utils import xt2large_noise
...
class TSPDiffusionDataset_v2(Dataset):
...
class TSPDiffusionDataset_v3(Dataset):
    def __init__(self, points, sparse):
        self.points = points
        self.sparse = sparse
    def __len__(self):
        return len(self.points)
    def __getitem__(self, idx):
        if self.sparse > 0:
          ...
          sparse_factor = self.sparse
          kdt = KDTree(points, leaf_size=30, metric='euclidean')
          _, idx_knn = kdt.query(points, k=sparse_factor, return_distance=True)
...
class TSPModel_v2(COMetaModel):
    ...
    def categorical_denoise_step(self, points, xt, t, device = 0, edge_index=None, target_t=None):
        with torch.no_grad():
            t = torch.from_numpy(t).view(1)
            x0_pred = self.forward(
                points.float().to(device),
    ...
    def test(self, gpu = 0):
        ...
        if self.dataset is None:
            raise ValueError("Data not set. Use set_data() before testing.")
        
        _, points = next(iter(self.dataloader))
        solution_tensor = torch.zeros((self.args.sequential_sampling, len(self.dataset), points.shape[1], points.shape[1]), dtype=torch.float, device='cpu')
        solution_points = np.zeros((self.args.sequential_sampling, len(self.dataset), points.shape[1], 2)) 
        
        for ss in range(self.args.sequential_sampling):
            for batch in self.dataloader:
                real_batch_idx, points = batch
                np_points = points.cpu().numpy() # [0]?
                xt = torch.randn((points.shape[0], points.shape[1], points.shape[1]), dtype=torch.float, device = device)
                ...
                with tqdm(total=steps) as pbar:
                    for i in range(steps):
                        ...
                        t1 = np.array([t1]).astype(int)
                        t2 = np.array([t2]).astype(int)
                        xt = self.categorical_denoise_step(points, xt, t1, device, target_t=t2)
                        pbar.update()
                solution_tensor[ss, real_batch_idx] = xt.cpu()
                solution_points[ss, real_batch_idx, : , : ] = np_points

        return solution_tensor, solution_points

    def test_multiscale(self, model2, enhanced_points2, scale1_subcount, scale2_times, device = 0, pre_prop = 0.9, noise_prop = 0.1):
        ...
        scale1_main, scale1_extend = int(10000//scale1_subcount), scale_1extend
        scale2_main, scale2_extend = scale1_main*scale2_times, enhanced_points2[0].shape[0]
        enhanced_points2 = torch.tensor(enhanced_points2, device=device)

        assert B%scale2_times == 0, "the batch size should times of multiscale"

        solution_tensor = torch.zeros((self.args.sequential_sampling, len(self.dataset)//scale2_times, scale2_extend, scale2_extend), dtype=torch.float, device='cpu')
        solution_points = np.zeros((self.args.sequential_sampling, len(self.dataset)//scale2_times, scale2_extend, 2)) 
        
        for ss in range(self.args.sequential_sampling):
            print(f'{ss} sequential sampling')
            batch_num = 0
            for batch in self.dataloader:
                real_batch_idx, points = batch
                np_points = points.cpu().numpy()
                
                xt = torch.randn((B, scale_1extend, scale_1extend), dtype=torch.float, device = device)

                if self.args.parallel_sampling > 1:
                    points = points.repeat(self.args.parallel_sampling, 1, 1)
                    xt = xt.repeat(self.args.parallel_sampling, 1, 1)

                if self.diffusion_type == 'gaussian':   xt.requires_grad = True
                else:                                   xt = (xt > 0).long()

                total_steps =  self.args.inference_diffusion_steps
                pre_step1   = int(total_steps * pre_prop)
                pre_step2  = int(total_steps * 0.05)
                time_schedule = InferenceSchedule(
                    inference_schedule=self.args.inference_schedule,
                    T=self.args.diffusion_steps,
                    inference_T=total_steps
                )

                groups     = B // scale2_times
                new_index = torch.tensor(list(range(batch_num, batch_num + groups)), dtype=torch.long)
                
                with tqdm(total=total_steps) as pbar:
                    for i in range(pre_step1):
                        t1, t2 = time_schedule(i)
                        t1 = np.array([t1]).astype(int)
                        t2 = np.array([t2]).astype(int)
                        xt = self.categorical_denoise_step(points, xt, t1, device, target_t=t2)
                    ...
                    intra_noise_prop = 0.36
                    inter_noise_prop = 0.3
                    xt = xt[:, :scale1_main, :scale1_main]
                    xt_grouped = xt.view(groups, scale2_times, scale1_main, scale1_main)
                    
                    
                    xt_groups = xt2large_noise(groups, xt_grouped, [scale2_times, scale2_main, scale2_extend,],intra_noise_prop, inter_noise_prop)
                ...
                solution_tensor[ss, new_index] = xt_2.cpu()
                solution_points[ss, new_index, : , : ] = np_points_2
                batch_num += groups
                # Save Solution Heatmaps
